...
local function detect_git_tui(explicit)
  ...
  local candidates = explicit and { explicit } or GIT_TUIS
  local query = "command -v " .. table.concat(candidates, " ") .. " 2>/dev/null | head -1"
...
function M.git_selector(window, pane, deps)
  ...
  if not git_tui then
    toast(window, L.git_no_tui)
    return
  end

  local env = {}
  if deps.opts.lazygit_config_file then
    env.LG_CONFIG_FILE = deps.opts.lazygit_config_file
  elseif git_tui:match("lazygit") then
    env.LG_CONFIG_FILE = deps.opts.plugin_dir .. "/plugin/lazygit.yml"
  end

  window:perform_action(
    act.SpawnCommandInNewTab({ args = { "/bin/sh", "-lc", git_tui }, cwd = git_root, set_environment_variables = env }),
    pane
  )
end
