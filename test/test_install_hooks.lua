...
test("再適用：.bak は初回の原本を保ち、2回目の applied で上書きしない", function()
  local home = H.tmp_dir()
  os.execute("mkdir -p " .. sh(home .. "/.claude"))
  local original = '{"model":"opus","hooks":{"Stop":[{"hooks":[{"type":"command","command":"echo other"}]}]}}'
  H.write_file(home .. "/.claude/settings.json", original)

  -- 1回目: dirA で applied → .bak に原本を退避。
  H.assert_match(run(home, "/tmp/dirA", { "claude" }), "applied claude")
  H.assert_not_nil(H.read_file(home .. "/.claude/settings.json.bak"), "初回適用で .bak を作る")
  -- 2回目: dir を変えると command パスが変わり再び applied。.bak は上書きされない。
  H.assert_match(run(home, "/tmp/dirB", { "claude" }), "applied claude")
  H.assert_eq(
    H.read_file(home .. "/.claude/settings.json.bak"),
    original,
    ".bak は最初の原本のまま (改変済み内容で上書きしない)"
  )
  cleanup(home)
end)
