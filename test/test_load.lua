...
test("正常系：service/agents/*.lua を自動発見し id 昇順で全エージェントを登録する", function()
  wezterm._events = {}
  local init = load_mod("init")
  local config = wezterm.config_builder()

  -- run_child_process モックは検出失敗 (nil) を返すため、全発見エージェントが登録される。
  init.apply(config, { plugin_dir = H.plugin_dir })

  local ids = {}
  for _, impl in ipairs(init.agent.all()) do
    ids[#ids + 1] = impl.id
  end
  -- agents/ にファイルを足せばここに自動で増える (手動リスト無しの担保)。
  H.assert_eq(table.concat(ids, ","), "claude,codex,cursor,gemini")
end)
