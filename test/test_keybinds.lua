...
test("退行検出：主要セレクタ callback が nil 参照なく実行できる", function()
  ...
  local opts = {
    ...
    disabled_keybinds = {},
    keybinds = {},
    -- manager 召喚 (M) が読むワークスペース別 manager ペインファイル。存在しないパスでも read は nil。
    manager_file = H.tmp_dir() .. "/manager.json",
    manager_agent = "claude",
    manager_command = "claude",
  }
  local deps = {
    opts = opts,
    agent = agent,
    ...
    manager = H.load_mod("state/manager"), -- manager 召喚の結線で参照
  }
  ...
  -- workspace/worktree/agent/help/editor/pin/manager の各 callback (= 各サブモジュール境界) を踏む
