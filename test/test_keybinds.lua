...
test("退行検出：主要セレクタ callback が nil 参照なく実行できる", function()
  ...
  local opts = {
    ...
    disabled_keybinds = {},
    keybinds = {},
    -- command_center (M) が読む監督集合ファイル。存在しないパスでも read は空集合を返す。
    managed_file = H.tmp_dir() .. "/managed.json",
    orchestrator_file = H.tmp_dir() .. "/orchestrator.json",
  }
  local deps = {
    opts = opts,
    agent = agent,
    ...
    managed = H.load_mod("state/managed"), -- command_center の結線で参照
  }
  ...
  -- workspace/worktree/agent/help/editor/pin/command_center の各 callback (= 各サブモジュール境界) を踏む
  for _, target in ipairs({ "S", "X", "A", "H", "E", "P", "M" }) do
