...
`mcp/` に Go 製 MCP サーバーがある。Claude Code 等の AI エージェントから WezTerm のワークスペース・worktree・エージェントを操作する。

**セットアップ:**
```bash
cd mcp && go build -o wezterm-mcp .
claude mcp add -s user wezterm ./mcp/wezterm-mcp
```
