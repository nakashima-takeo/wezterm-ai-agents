...
claude/codex/gemini が同じ MCP サーバー・supervise スキルを共有する **AI エージェント側の配布物**。WezTerm 側の Lua プラグイン (`plugin/`) とは別ランタイムで同一リポジトリに同居する。

**ルートの非対称 (重要):** gemini の拡張ルートは **repo ルート**、claude/codex は `./agent-plugin` サブディレクトリ。両者を分けるのは必然で、(1) `gemini extensions install <repo-url>` は repo ルート直下の `gemini-extension.json` しか探さない (サブパス不可)、(2) claude/gemini は共に `<root>/hooks/hooks.json` を自動検出するが claude は gemini 専用イベント (BeforeAgent/AfterAgent) を拒否してプラグイン全体のロードに失敗する。よって gemini 専用の `hooks/hooks.json` は claude のルート (`agent-plugin/`) から物理的に分離する必要がある。共有資産 (supervise スキル・agent_status.sh) は repo ルートから `agent-plugin/` 内の実体へ symlink して単一ソースを保つ。
...
├── skills/                     → symlink → agent-plugin/skills (supervise を gemini に供給)
