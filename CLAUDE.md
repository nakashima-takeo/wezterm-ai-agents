...
```
`claude-plugin/.mcp.json` の command は共有バイナリ `$XDG_STATE_HOME/wezterm-ai-agents/bin/wezterm-mcp` を指す。これは WezTerm プラグインが起動時に `hooks/install_mcp.sh` で用意する (`v*` Release からプリビルドを DL、無ければ `go build` フォールバック ＝ dev のみ要 Go)。**claude/codex/gemini が同じ1つのバイナリを共有**するため利用環境に Go は不要。ローカル開発は `claude --plugin-dir ./claude-plugin` か `/plugin marketplace add ./` で確認できる。
