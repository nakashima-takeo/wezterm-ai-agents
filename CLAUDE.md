...
```
`claude-plugin/.mcp.json` が MCP サーバーを宣言し、`hooks/hooks.json` の SessionStart フックが `go build` でバイナリを用意する (要 Go 1.26 以上)。バイナリは未追跡 (gitignore) なので**初回導入時はビルド完了より先に MCP 接続が試行されることがあり、その場合は Claude Code を再起動するとツールが有効になる**。ローカル開発は `claude --plugin-dir ./claude-plugin` か `/plugin marketplace add ./` で確認できる。
