...
4. `hooks/install_hooks.sh` の `case "$id"` にエージェント→設定ファイルのパスと登録イベントの spec を追記する (起動時自動設定の対象に含めるため。ここを忘れると自動設定だけ漏れる)。設定ファイル構造が Claude/Codex/Gemini と異なる場合は style 分岐も要追加。
