...
claude/codex/gemini が同じ MCP サーバー・manager スキルを共有する **AI エージェント側の配布物**。WezTerm 側の Lua プラグイン (`plugin/`) とは別ランタイムで同一リポジトリに同居する。

**ルートの非対称 (重要):** gemini の拡張ルートは **repo ルート**、claude/codex は `./agent-plugin` サブディレクトリ。両者を分けるのは必然で、(1) `gemini extensions install <repo-url>` は repo ルート直下の `gemini-extension.json` しか探さない (サブパス不可)、(2) claude/gemini は共に `<root>/hooks/hooks.json` を自動検出するが claude は gemini 専用イベント (BeforeAgent/AfterAgent) を拒否してプラグイン全体のロードに失敗する。よって gemini 専用の `hooks/hooks.json` は claude のルート (`agent-plugin/`) から物理的に分離する必要がある。共有資産 (manager スキル・agent_status.sh) は repo ルートから `agent-plugin/` 内の実体へ symlink して単一ソースを保つ。

```
repo-root/
├── gemini-extension.json       — gemini マニフェスト (mcpServers インライン)。repo ルート必須
├── hooks/hooks.json            — gemini フック (BeforeAgent/AfterAgent)。gemini 固定の自動検出先
├── hooks/agent_status.sh       → symlink → agent-plugin/hooks/agent_status.sh
├── commands/manager.toml      — gemini の /manager 決定起動コマンド
├── skills/                     → symlink → agent-plugin/skills (manager を gemini に供給)
└── agent-plugin/               — claude/codex プラグインルート + 共有資産の実体
    ├── .claude-plugin/plugin.json — claude マニフェスト (skills + .mcp.json + hooks/claude-hooks.json)
    ├── .codex-plugin/plugin.json  — codex マニフェスト (skills + .mcp.json + hooks/codex-hooks.json)
    ├── .mcp.json                  — 共有 MCP 定義 (claude/codex が参照、sh -c で共有バイナリ起動)
    ├── hooks/{claude,codex}-hooks.json — 各社フック (event→state)
    ├── hooks/agent_status.sh      — 共有フック本体 (単一ソース)
    ├── skills/manager/SKILL.md    — 3 社共有の manager スキル本体 (単一ソース)
    │   └── agents/openai.yaml      — codex 専用ガード (allow_implicit_invocation:false)
    └── mcp/                        — 共有 Go MCP サーバー
```
各社の manager 決定起動: claude=`/wezterm-ai-agents:manager`、codex=`$manager`、gemini=`/manager`。Lua の `build_command` (`manager_agent` で選択) が各社流に組み立てる。`disable-model-invocation` は codex が拒否するため SKILL.md には付けず、description と codex の openai.yaml で自発起動を抑止する。
...
`get_agents` は全エージェントペインとライブ状態を返す (`pane_ids` を渡すと対象を絞れる)。`wait_for_event` は (`pane_ids` で指定した) エージェントの状態変化／出現・消滅までブロックし差分を返す (manager の管理ループ用。呼び出し側にはイベント駆動、内部は 300ms の状態ファイルポーリング)。状態ファイルは `WEZTERM_UNIX_SOCKET` 由来の gui_pid 名前空間配下を読む (フック・Lua と同一規則)。**人手トグルの監督集合は持たず、manager が管理対象を自分で選ぶ** (`get_agents` で見渡し、`wait_for_event(pane_ids=…)` で選んだ集合だけ見守る)。

**受付マネージャーはワークスペース単位**: `CMD+SHIFT+M` で現在のワークスペースの manager を召喚する (居れば前面化、居なければ起動してフォーカス)。manager は依頼をタブへ委譲し、必要と判断すれば自分が spawn したタブも既存タブも見守る (管理は manager の裁量)。Lua は manager ペインを `manager.json` (`{"<ws>":pane_id}` でキー化、タブを専用色で表示) で追跡し、召喚時に `WEZTERM_AGENT_WORKSPACE` を渡す。
...
3. claude/codex は `agent-plugin/hooks/<id>-hooks.json` に event→state を定義しマニフェストの `hooks` フィールドで参照する。gemini は repo ルートの `hooks/hooks.json` を固定で自動探索する (claude のルートと衝突させないため repo ルート側に置く)。`hooks/install_plugins.sh` の `case "$id"` に各社の install コマンドを追記する (起動時自動導入の対象に含めるため)。
