...
```
`claude-plugin/.mcp.json` が MCP サーバーを宣言し、`hooks/hooks.json` の SessionStart フックが `go build` でバイナリを用意する (要 Go 1.26 以上)。ローカル開発は `claude --plugin-dir ./claude-plugin` か `/plugin marketplace add ./` で確認できる。
